 ...
 <?php 
	...
	// echo (null + ',' + null)==',';
 	if(isset($_GET["search"])){
		...
		$mode= $_GET["mode"];
		$offset= $_GET["offset"];
		if(($_GET["center"]) != 'null' && $_GET['center']!=null){
			//echo 'has center';
			$center = $_GET["center"];
			$response = $fb->get("/search?q=$key&type=$mode&limit=25&offset=$offset&center=$center&fields=id, name, picture.width(700).height(700), albums, posts");
		}else{
			//echo 'no center';
			$response = $fb->get("/search?q=$key&type=$mode&limit=25&offset=$offset&fields=id, name, picture.width(700).height(700), albums, posts");
